...
```bash
git clone <repo> moricar && cd moricar
...
| `MORICAR_DEFAULT_RADIUS_KM` | Radius pre-selected in the search module (default 10). |
| `MORICAR_MAX_RADIUS_KM` | Hard cap enforced by validation (default 50). |
| `MORICAR_REVIEW_ON_SENSITIVE_EDIT` | When true, editing identity/location/services returns an approved profile to review. |
| `MORICAR_WHATSAPP_TEMPLATE` | Prefilled message. Supports `:app` and `:service`. |
| `MORICAR_SUPPORT_EMAIL` / `_WHATSAPP` | Shown in the footer and contact page. |
...
- **1 admin** — `ADMIN_EMAIL` (default `admin@moricar.mu`). Set `ADMIN_PASSWORD` in `.env`, or leave it
...
Backend tests run against **MySQL** (`moricar_testing`, configured in `phpunit.xml`) because search relies
on SQL trigonometric functions. Create it once:

```bash
docker exec moricar-mysql mysql -uroot -p"$DB_PASSWORD" -e 'CREATE DATABASE IF NOT EXISTS moricar_testing;'
```
