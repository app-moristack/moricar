...
const CONTENT: Record<string, { title: string; intro: string; sections: Section[] }> = {
  about: {
    title: 'About MoriCar',
    intro:
      'MoriCar is a Mauritian directory for construction, renovation, maintenance and home services. We exist so that finding a reliable tradesperson near you does not depend on who you happen to know.',
    sections: [
      {
  ...
  terms: {
    title: 'Terms of use',
    intro:
      'These terms govern the use of MoriCar. By searching the directory or registering a profile, you accept them.',
    sections: [
      {
        heading: 'What MoriCar is',
        body: [
          'MoriCar is a directory. We introduce customers to professionals. We are not a party to any agreement, quotation, or work carried out between them, and we do not supervise that work.',
  ...
  privacy: {
    ...
    intro: 'This explains what MoriCar collects, why, and what we do not do with it.',
    sections: [
      {
...
export default function StaticPage({ slug }: { slug: string }) {
  ...
  const supportWhatsapp = buildWhatsappUrl({
    ...
    serviceName: 'the MoriCar directory',
  })
