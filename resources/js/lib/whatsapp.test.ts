...
describe('buildWhatsappMessage', () => {
  it('fills the app name and the service into the template', () => {
    expect(buildWhatsappMessage({ serviceName: 'Plumbing', appName: 'MoriCar', template: TEMPLATE })).toBe(
      'Hello, I found your profile on MoriCar. I am looking for help with plumbing.',
    )
  })

  it('falls back to a generic service when none is known', () => {
    expect(buildWhatsappMessage({ appName: 'MoriCar', template: TEMPLATE })).toBe(
      'Hello, I found your profile on MoriCar. I am looking for help with a project at home.',
    )
  })

  it('treats an empty service name as unknown', () => {
    expect(buildWhatsappMessage({ serviceName: '', appName: 'MoriCar', template: TEMPLATE })).toContain(
      'a project at home',
    )
  })
})
...
describe('buildWhatsappUrl', () => {
  it('builds a wa.me link with the prefilled message', () => {
    ...
    expect(url).toBe(
      'https://wa.me/23057654321?text=Hello%2C%20I%20found%20your%20profile%20on%20MoriCar.%20I%20am%20looking%20for%20help%20with%20tiling.',
    )
  })

  it('strips formatting characters from the number', () => {
    const url = buildWhatsappUrl({ number: '+230 5765 4321', appName: 'MoriCar', template: TEMPLATE })
