...
<head>
    ...
    <meta name="apple-mobile-web-app-title" content="MoriCar">
    ...
    <script nonce="{{ Vite::cspNonce() }}">
        window.__MORICAR__ = @json($bootstrap);
    </script>

    @viteReactRefresh
    @vite(['resources/css/moricar.css', 'resources/js/main.tsx'])
</head>
<body>
    ...
    @if ($meta->noscript)
        <noscript>
            <div style="max-width:48rem;margin:0 auto;padding:2rem 1rem;font-family:system-ui,sans-serif">
                {!! $meta->noscript !!}
                <p><a href="/">MoriCar</a> needs JavaScript for search and messaging.</p>
            </div>
        </noscript>
    @endif
</body>
