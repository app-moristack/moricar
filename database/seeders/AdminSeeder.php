...
class AdminSeeder extends Seeder
{
    ...
    {
        $email = config('moricar.admin.email');
        $password = config('moricar.admin.password') ?: Str::password(16);
        ...
        DB::transaction(function () use ($email, $password): void {
            ...
            $admin = User::updateOrCreate(
                ['email' => $email],
                [
                    'name' => 'MoriCar Administrator',
                    'phone' => PhoneNumber::parse(config('moricar.support_whatsapp'))->e164,
        ...
        if (! config('moricar.admin.password')) {
