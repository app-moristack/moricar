...
class SecurityHardeningTest extends TestCase
{
    ...
    public function test_profile_links_and_oversized_images_are_rejected(): void
    {
        ...
        Storage::fake('public');
        config(['moricar.uploads.max_dimension' => 100]);
    ...
    public function test_unconfigured_production_smtp_does_not_create_reset_tokens(): void
    {
        $this->app->instance('env', 'production');
        ...
        $user = User::factory()->create();
        $known = $this->postJson('/api/v1/forgot-password', ['email' => $user->email])->assertOk()->json();
        $unknown = $this->postJson('/api/v1/forgot-password', ['email' => 'unknown@example.mu'])->assertOk()->json();
        $this->assertSame($known, $unknown);
        Notification::assertNothingSent();
        $this->assertDatabaseMissing('password_reset_tokens', ['email' => $user->email]);
    }

    public function test_transport_failures_return_generic_response_without_logging_credentials(): void
    {
        Password::shouldReceive('sendResetLink')->once()->andThrow(
            new TransportException('secret transport details'),
        );
        Log::shouldReceive('warning')->once()->with(
            'Password recovery delivery failed.',
            ['exception_type' => TransportException::class],
        );
        $this->postJson('/api/v1/forgot-password', ['email' => 'someone@example.mu'])
            ->assertOk()->assertJson(['message' => 'If an account matches this email, you will receive password reset instructions.']);
    }
}
