...
class SeoAndSpaTest extends TestCase
{
    ...
    public function test_an_approved_profile_gets_its_own_title_and_structured_data(): void
    {
        ...
        $this->get('/providers/'.$provider->slug)
            ->assertOk()
            ->assertSee('<title>Volt Maurice — Electrician in Curepipe | MoriCar</title>', false)
    ...
    {
        $this->get('/')
            ->assertSee('window.__MORICAR__', false)
    ...
    {
        $this->get('/privacy')->assertSee('<title>Privacy policy — MoriCar</title>', false);
        $this->get('/terms')->assertSee('<title>Terms of use — MoriCar</title>', false);
    }
}
